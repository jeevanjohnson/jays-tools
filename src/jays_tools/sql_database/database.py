...
class SQLDatabase:
    ...
    async def _create_table(
        self, db: aiosqlite.Connection, table: Type[MigratableSQLModel]
    ) -> None:
        ...
        create_table_sql = (
            f"CREATE TABLE IF NOT EXISTS {table.get_table_name()} ({columns_sql})"
        )
    ...
    async def find(
        self, schema: Type[T], where: Union[dict[str, Any], Filter, None] = None
    ) -> list[T]:
        ...
        if where is None:
            ...
            raise TypeError(
                f"where must be Filter, dict, or None, got {type(where)}")
    ...
    async def insert(self, entry: T) -> T:
        ...
        insert_sql = (
            f"INSERT INTO {table_name} ({fields_sql}) VALUES ({placeholders_sql})"
        )
    ...
    async def _insert_with_connection(self, db: aiosqlite.Connection, entry: T) -> T:
    ...
    async def _update_with_connection(self, db: aiosqlite.Connection, entry: T) -> T:
